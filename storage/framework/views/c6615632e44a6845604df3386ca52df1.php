...
<script type="application/ld+json">
{
    ...
    "@type": "FAQPage",
    "name": "<?php echo e(__('help.faq.page_title')); ?> — reception-par-type.ch",
    "url": "<?php echo e(url()->current()); ?>",
    "inLanguage": "<?php echo e(app()->getLocale()); ?>",
    "mainEntity": [
        <?php $__currentLoopData = __('help.faq.questions'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $faq): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        {
            "@type": "Question",
            "name": <?php echo e(json_encode($faq['question'], JSON_UNESCAPED_UNICODE)); ?>,
            "acceptedAnswer": {
                "@type": "Answer",
                "text": <?php echo e(json_encode(strip_tags($faq['answer']), JSON_UNESCAPED_UNICODE)); ?>
...
>
    ...
    <div class="relative mb-8">
        ...
        <input
            ...
            placeholder="<?php echo e(__('help.faq.search_placeholder')); ?>"
            class="
    ...
    >
        ...
        <?php $__currentLoopData = ['general', 'wheels', 'pricing', 'account']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <button
                role="tab"
                x-on:click="activeCategory = '<?php echo e($cat); ?>'"
                x-bind:aria-selected="activeCategory === '<?php echo e($cat); ?>'"
                x-bind:class="{
                    'bg-astra text-white border-astra shadow-sm shadow-astra/20': activeCategory === '<?php echo e($cat); ?>',
                    'bg-white dark:bg-marine/30 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-white/10 hover:border-astra/30': activeCategory !== '<?php echo e($cat); ?>'
                }"
                class="px-4 py-1.5 rounded-lg text-xs font-medium border transition-all duration-150"
            >
                <?php echo e(__("help.faq.categories.{$cat}")); ?>
    ...
    <div
        class="space-y-2"
        role="list"
        aria-label="<?php echo e(__('help.faq.page_title')); ?>"
    >
        <?php $__currentLoopData = __('help.faq.questions'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $faq): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div
                x-show="isVisible('<?php echo e($faq['cat']); ?>', <?php echo e(json_encode($faq['question'])); ?>, <?php echo e(json_encode(strip_tags($faq['answer']))); ?>)"
            ...
            >
                ...
                >
                    <span class="text-sm font-medium text-slate-900 dark:text-white leading-snug pr-2">
                        ...
                        <span x-html="highlight(<?php echo e(json_encode($faq['question'])); ?>)"></span>
                    </span>
        ...
        >
            ...
            <p class="text-sm"><?php echo e(__('help.faq.no_results')); ?></p>
        </div>
    </div>
...
function faqManager() {
    return {
        activeCategory: 'all',
        searchQuery:    '',
        visibleCount:   <?php echo e(count(__('help.faq.questions'))); ?>,
         ...
         */
        updateCount() {
            const questions = <?php echo json_encode(__('help.faq.questions'), 15, 512) ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\DiskFiles\Projects\Web\upwork_Invoice_Ninja\reception-par-type\resources\views/pages/faq.blade.php ENDPATH**/ ?>